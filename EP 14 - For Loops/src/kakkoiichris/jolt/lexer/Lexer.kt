...
class Lexer(private val source: Source) : Iterator<Token<*>> {
    ...
    private var interpolateLevel = 0

    private val interpolating get() = interpolateLevel > 0
    ...
    override fun next(): Token<*> {
        while (!atEndOfFile()) {
            ...
            return when {
                ...
                interpolating && match('}') -> continueInterpolation()
    ...
    private fun matchAny(vararg chars: Char) =
        chars.any(::match)
     ...
     */
    private fun mustSkip(char: Char, errorMessage: String = "LEXER_CORRUPTED") {
        if (!skip(char)) {
            val context = here()
     ...
     */
    private fun mustSkip(string: String, errorMessage: String = "LEXER_CORRUPTED") {
        if (!skip(string)) {
    ...
    private fun checkEndOfFile(message: String, context: Context = here()) {
        if (atEndOfFile()) {
            joltError(message, source.getLine(context.row), context)
        }
    }
    ...
    private fun string(): Token<TokenType.Value<JoltValue.String>> {
        val start = here()
        ...
        val result = buildString {
            while (!skip('"')) {
                checkEndOfFile("Unclosed string", start)
        ...
        return Token(context, TokenType.Value(JoltValue.String(result)))
    }

    private fun interpolation(): Token<TokenType.OpenInterpolate> {
        val start = here()
        ...
        val result = buildString {
            while (!skip('{')) {
                checkEndOfFile("Unclosed string interpolation", start)
        ...
        return Token(context, TokenType.OpenInterpolate(JoltValue.String(result)))
    }

    private fun continueInterpolation(): Token<*> {
        val start = here()
        ...
        if (closed) {
            ...
            return Token(context, TokenType.CloseInterpolate(JoltValue.String(result)))
        }

        return Token(context, TokenType.MidInterpolate(JoltValue.String(result)))
    }

    private fun escape(isInterpolated: Boolean): Char {
        val start = here()
        ...
        return when {
            ...
            match('(')                  -> unicodeName()
                ...
                joltError("Invalid character escape sequence '\\${peek()}'", source.getLine(context.row), context)
            }
        }
    }

    private fun hexcode(length: Int): Char {
        val start = here()

        val code = buildString {
            repeat(length) {
                checkEndOfFile("Unfinished hexadecimal escape of length '$length'", start)
    ...
    private fun unicodeName(): Char {
        ...
        val result = buildString {
            while (!skip(')')) {
                checkEndOfFile("Unclosed named unicode escape", start)

                take()
            }
        }

        return Character.codePointOf(result).toChar()
    }
