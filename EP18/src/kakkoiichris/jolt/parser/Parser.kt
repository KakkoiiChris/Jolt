...
class Parser(private val source: Source, private val lexer: Lexer) {
    ...
    private fun assign(): Expr {
        ...
        if (match(TokenType.Symbol.EQUAL)) {
            ...
            if (expr is Expr.GetMember) {
                ...
                return Expr.SetMember(context, expr.target, expr.member, value)
            }
    ...
    private fun postfix(): Expr {
        ...
        while (matchAny(TokenType.Symbol.LEFT_SQUARE, TokenType.Symbol.LEFT_PAREN, TokenType.Symbol.PERIOD)) {
            val (_, type) = get<TokenType.Symbol>()
            ...
            expr = when (type) {
                ...
                TokenType.Symbol.PERIOD      -> access(expr)
    ...
    private fun access(target: Expr): Expr.GetMember {
        val member = name()

        val context = target.context..here()

        return Expr.GetMember(context, target, member)
    }
