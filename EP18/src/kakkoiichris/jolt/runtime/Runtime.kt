...
class Runtime(private val source: Source) : Stmt.Visitor<Unit>, Expr.Visitor<JoltValue<*>> {
     ...
     * @return [Unit]
     */
    override fun visitEmptyExpr(expr: Expr.Empty) = JoltUnit
    ...
    override fun visitGetMemberExpr(expr: Expr.GetMember): JoltValue<*> {
        val target = visit(expr.target)
    ...
    override fun visitSetMemberExpr(expr: Expr.SetMember): JoltValue<*> {
        val target = visit(expr.target)
