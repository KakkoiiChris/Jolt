...
class Parser(private val source: Source, private val lexer: Lexer) {
    ...
    private fun forStmt(): Stmt.For {
        ...
        mustSkip(TokenType.Symbol.LEFT_PAREN, "Pointer must be preceded by a left parenthesis")

        val pointer = nameExpr()

        mustSkip(TokenType.Symbol.COLON, "Pointer and iterable must be separated by a colon")

        val iterable = expr()

        mustSkip(TokenType.Symbol.RIGHT_PAREN, "Condition must end with a right parenthesis")
    ...
    private fun terminalExpr() = when {
        ...
        match(TokenType.Symbol.LEFT_SQUARE) -> listExpr()
    ...
    private fun listExpr(): Expr.ListLiteral {
        val start = here()
        ...
        if (!skip(TokenType.Symbol.RIGHT_SQUARE)) {
            ...
            mustSkip(TokenType.Symbol.RIGHT_SQUARE, "List literal must end with a closing square bracket")
        }
